    ...
    public partial class Form2 : Form
    {
        ...
        private void btnConvertDecimal_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                //The Decimal Number will be divided into 2 until the number to be converted is equal to zero
                while (number != 0) 
                {
                    quotient = number / 2;
                ...
                //For loop is used in displaying the string identifier, 'remainder' to get the equivalent Binary Number
                ...
                {
                    txtBinary.Text = txtBinary.Text + remainder[i];
                }
            }
        }
        ...
        private void btnConvertBinary_Click(object sender, EventArgs e)
        {
            if (txtBinary.Text == "")
            {
                ...
                //Condition to check if the textbox for Binary Number is empty
                MessageBox.Show("INPUT BINARY NUMBER");
            }
            else
            {
                int remainder, dec = 0, power = 1;

                number = int.Parse(txtBinary.Text);
                //While loop is to perform the method of multiplying each digit in the Binary number to its positional value.
                //Then adding it to its previous value
                //The Binary Number will be divided into 10 until the number to be converted is equal to zero
                while (number != 0)
                {
                    quotient = number / 10;
                    remainder = number % 10;
                    ...
                    number = quotient;
                    power = power * 2; //We'll be dealing with the number system that has the base number 2
                }
                txtDecimal.Text = dec.ToString(); //Displaying the last value accumulated in integer identifier 'dec' which is the equivalent Decimal Number of the inputted Binary Number
            }
        }
        ...
        {
            //The Decimal Number's textbox can't receive any input from the user since we'll be dealing with Binary Number
            if (txtBinary.Text != "")
            {
        ...
        {
            //User can only use the numbers 0, 1 and the backspace function in inputting the Binary Number
        ...
        {
            //The Binary Number's textbox can't receive any input from the user since we'll be dealing with Decimal Number
            if (txtDecimal.Text != "")
            {
